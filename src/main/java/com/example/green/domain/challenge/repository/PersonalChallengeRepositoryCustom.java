...
/**
 * 개인 챌린지 조회를 위한 커스텀 리포지토리
 * QueryDSL을 사용하여 커서 기반 페이지네이션을 구현합니다.
 */
public interface PersonalChallengeRepositoryCustom {

	/**
	 * 개인 챌린지 목록을 커서 기반으로 조회합니다.
	 * @param cursor 마지막으로 조회한 챌린지의 ID
	 * @param size 조회할 챌린지 수
	 * @param status 챌린지 상태
	 * @param now 현재 시간
	 * @return 커서 템플릿에 담긴 개인 챌린지 목록
	 */
	...
	CursorTemplate<Long, AdminPersonalChallengeListResponseDto> findAllForAdminByCursor(Long cursor, int size);
}
