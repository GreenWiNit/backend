...
class GrowthControllerTest extends BaseControllerUnitTest {
	...
	void getGrowthSuccess() {
		// given
		LoadGrowthResponse growthResponse = new LoadGrowthResponse(
			1L,
			Level.TREE,
			Level.SAPLING,
		...
		when(growthService.loadGrowth(anyLong()))
		...
		GrowthController controller = new GrowthController(growthService, plantItemService);

		// when
		ApiTemplate<LoadGrowthResponse> response = controller.getGrowth(principal);
